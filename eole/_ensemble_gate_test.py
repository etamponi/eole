...
class EnsembleGateTest(unittest.TestCase):
    ...
    def setUp(self):
        self.gate = EnsembleGate(
            experts=[
                LocalExpert(FakeEstimator(n_labels=2, label_cycle=[0, 1], prob=0.7), [0, 0], None),
                LocalExpert(FakeEstimator(n_labels=2, label_cycle=[1, 1], prob=0.9), [-1, 0], None),
                LocalExpert(FakeEstimator(n_labels=2, label_cycle=[1, 0], prob=0.6), [1, 0], None)
            ],
